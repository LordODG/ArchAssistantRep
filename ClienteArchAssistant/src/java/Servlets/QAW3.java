...
public class QAW3 extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        String guardar = request.getParameter("btnQaw3Guardar");
        String continuar = request.getParameter("btnQaw3Continuar");
        String regresar = request.getParameter("btnQaw3anterior");
        String subir = request.getParameter("btnQaw3subir");
        
        if (guardar != null)
        {
            ArchAssistantBean archB = new ArchAssistantBean();
            Proyecto proy = (Proyecto)request.getSession().getAttribute("proyectoActual");
            Rationaleqaw ratq = archB.RationaleQAW(proy.getProID(), "qaw3");
            if (ratq == null)
            {
